    ...
    public static Document applyTest(Document document, QnTest qnTest) throws JDOMException, IOException {
        ...
        for (OpenClass openClass : qnTest.getOpenClasses()) {
            ...
            if (openClass instanceof OpenClassExponential) {
                ...
                /*double newLambdaValue = openClassExp.getInterarrivaldistributionLambda();
                Element value = distrPar.getChildren("subParameter").stream().filter(s -> s.getAttributeValue("name").equals("lambda")).findFirst().orElse(null).getChild("value");
                value.setText(String.valueOf(newLambdaValue));*/
                ...
                /*double newTvalue = openClassDet.getT();
                Element value = distrPar.getChildren("subParameter").stream().filter(s -> s.getAttributeValue("name").equals("t")).findFirst().orElse(null).getChild("value");
                value.setText(String.valueOf(newTvalue));*/
                ...
                /*double newMinValue = openClassUn.getMin();
                double newMaxValue = openClassUn.getMax();
                Element value = distrPar.getChildren("subParameter").stream().filter(s -> s.getAttributeValue("name").equals("min")).findFirst().orElse(null).getChild("value");
                value.setText(String.valueOf(newMinValue));
                value = distrPar.getChildren("subParameter").stream().filter(s -> s.getAttributeValue("name").equals("max")).findFirst().orElse(null).getChild("value");
                value.setText(String.valueOf(newMaxValue));*/
        ...
        if (closedClasses.size() > 0) {
            ...
            for (ClosedClass closedClass : closedClasses) {
                ...
                //TODO fix the code below for when there is no lambda value (ZeroServiceTimeStrategy)
//                Element lambdaValueElement = MutationOperator.getLambdaValueElement(selected);
//                lambdaValueElement.setText(thinkingTime);
            }
        }
